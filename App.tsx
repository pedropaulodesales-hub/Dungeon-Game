...
const EquipmentWindow: React.FC<{
        ...
        return (
            ...
            >
                <div 
                    ...
                    className={`w-14 h-14 flex items-center justify-center relative bg-[#0c0c0c] transition-all duration-300 group shadow-[inset_0_0_10px_rgba(0,0,0,0.8)]
                        ${item 
                            ? (item.rarity === 'LEGENDARY' ? 'border border-amber-600 shadow-[0_0_8px_rgba(245,158,11,0.2)]' : 
                               item.rarity === 'RARE' ? 'border border-blue-600 shadow-[0_0_8px_rgba(37,99,235,0.2)]' :
                               item.rarity === 'UNCOMMON' ? 'border border-green-600' : 'border border-[#444]') 
                            : 'border border-[#2a2a2a] hover:border-[#554023] hover:bg-white/5'}
                    `}
                >
                    {item ? (
                        <>
                        ...
                        <span className="text-[#333] text-[10px] font-black uppercase tracking-widest opacity-50">{slot.replace(/[0-9]/g, '').substring(0,3)}</span>
                    )}
                </div>
            </div>
        );
    ...
    return (
        <div className="absolute bottom-6 right-[448px] z-[100] w-[400px] h-[550px] flex flex-col bg-[#050505] border-2 border-[#3d3326] shadow-[0_0_0_1px_#000,0_10px_40px_rgba(0,0,0,0.95)] animate-in fade-in slide-in-from-right-10 duration-300">
             {/* Header */}
             <div className="h-10 bg-[linear-gradient(to_bottom,#1f1a14,#0a0806)] border-b border-[#3d3326] flex items-center justify-center relative select-none shrink-0">
                <div className="absolute left-2 text-[#5e4d35] text-xl">❖</div>
                <h2 className="text-[#c5a059] font-bold exocet-font text-lg tracking-widest drop-shadow-md">EQUIPMENT</h2>
                <div className="absolute right-2 text-[#5e4d35] text-xl">❖</div>
                <button onClick={onClose} className="absolute right-2 top-1/2 -translate-y-1/2 w-6 h-6 flex items-center justify-center text-[#5e4d35] hover:text-[#c5a059] transition-colors z-10 bg-black/50 border border-[#3d3326] rounded-sm text-xs">✕</button>
             </div>
             
             {/* Content */}
             <div className="flex-1 relative bg-[#080808] p-1">
                 ...
                 <div className="relative w-full h-full bg-gradient-to-b from-[#111] to-[#050505] border border-[#2a2a2a] shadow-inner">
                     ...
                     <div className="absolute top-[45%] left-[20%] transform -translate-x-1/2 -translate-y-1/2 pointer-events-none text-[8px] text-[#444] font-bold tracking-widest">MAIN</div>
                     <div className="absolute top-[45%] left-[80%] transform -translate-x-1/2 -translate-y-1/2 pointer-events-none text-[8px] text-[#444] font-bold tracking-widest">OFF</div>
                 ...
                 <div className="absolute bottom-0 left-0 right-0 bg-[#0a0806] border-t border-[#3d3326] p-3 flex justify-between px-6">
                      <div className="text-center"><div className="text-[9px] text-[#666] font-bold tracking-widest">ATK</div><div className="text-lg text-amber-500 font-bold exocet-font">{character.attack}</div></div>
                      <div className="text-center"><div className="text-[9px] text-[#666] font-bold tracking-widest">DEF</div><div className="text-lg text-slate-300 font-bold exocet-font">{character.defense}</div></div>
                      <div className="text-center"><div className="text-[9px] text-[#666] font-bold tracking-widest">MAG</div><div className="text-lg text-blue-400 font-bold exocet-font">{character.magic}</div></div>
                      <div className="text-center"><div className="text-[9px] text-[#666] font-bold tracking-widest">CRIT</div><div className="text-lg text-yellow-500 font-bold exocet-font">{character.crit}%</div></div>
                 </div>
            </div>
        </div>
    );
...
const InventoryWindow: React.FC<{
    ...
    // Calculate grid rows: 6 rows fixed, 5 columns = 30 slots
    const TOTAL_SLOTS = 30;

    return (
        <div className="absolute bottom-6 right-6 z-[100] w-[400px] h-[550px] flex flex-col bg-[#050505] border-2 border-[#3d3326] shadow-[0_0_0_1px_#000,0_10px_40px_rgba(0,0,0,0.95)] animate-in fade-in slide-in-from-right-10 duration-300">
             {/* Header */}
             <div className="h-10 bg-[linear-gradient(to_bottom,#1f1a14,#0a0806)] border-b border-[#3d3326] flex items-center justify-between px-3 relative select-none shrink-0">
                 <div className="text-[#5e4d35] text-xl">❖</div>
                 <div className="flex items-center gap-4">
                    <h2 className="text-[#c5a059] font-bold exocet-font text-lg tracking-widest drop-shadow-md">INVENTORY</h2>
                    <div className="h-4 w-[1px] bg-[#3d3326]"></div>
                    <span className="text-[#c5a059] font-bold text-sm tracking-wider flex items-center gap-1">{character.gold} <span className="text-[10px] text-[#8c7b5d]">GOLD</span></span>
                 </div>
                 <button onClick={onClose} className="w-6 h-6 flex items-center justify-center text-[#5e4d35] hover:text-[#c5a059] transition-colors bg-black/50 border border-[#3d3326] rounded-sm text-xs">✕</button>
             </div>

             {/* Content */}
             <div className="flex-1 relative bg-[#080808] p-1 flex flex-col" onDragOver={e => e.preventDefault()} onDrop={handleDropOnInventory}>
                 <div className="absolute inset-0 opacity-10 bg-[url('https://www.transparenttextures.com/patterns/dark-matter.png')] pointer-events-none"></div>
                 
                 <div className="relative flex-1 bg-gradient-to-b from-[#111] to-[#050505] border border-[#2a2a2a] shadow-inner p-2 overflow-y-auto custom-scrollbar">
                     {groupedInventory.length === 0 && (
                         <div className="absolute inset-0 flex items-center justify-center text-[#333] italic font-serif text-sm pointer-events-none">Stash is empty.</div>
                     )}
                         ...
                         {groupedInventory.map((group, idx) => {
                             ...
                             return (
                                 <div 
                                     key={`${item.id}_${idx}`}
                                     className={`
                                         relative aspect-square border cursor-grab active:cursor-grabbing group hover:bg-white/5 transition-colors
                                         flex items-center justify-center bg-[#0c0c0c] shadow-[inset_0_0_8px_rgba(0,0,0,0.8)]
                                         ${isLegendary ? 'border-amber-600/60' : 
                                           isRare ? 'border-blue-600/60' : 
                                           isUncommon ? 'border-green-600/60' : 'border-[#2a2a2a]'}
                                     `}
                                     draggable
                                 ...
                                 >
                                     ...
                                     {count > 1 && <div className="absolute bottom-0 right-0 bg-black/90 border border-[#333] text-white text-[9px] font-bold px-1 leading-none">{count}</div>}
                                     {item.type === 'POTION' && <div className="absolute top-1 right-1 w-1 h-1 rounded-full bg-red-500 shadow-[0_0_3px_red]"></div>}
                                 </div>
                             );
                         })}
                         {/* Fill remaining slots to look like a grid of 6 rows (30 total) */}
                         {Array.from({ length: Math.max(0, TOTAL_SLOTS - groupedInventory.length) }).map((_, i) => (
                             <div key={`empty_${i}`} className="aspect-square border border-[#1a1a1a] bg-[#080808] shadow-inner" />
                         ))}
                     </div>
                 </div>
                 
                 <div className="p-2 bg-[#0a0806] border-t border-[#3d3326] text-center shrink-0">
                    <p className="text-[9px] text-[#666] uppercase tracking-widest font-bold">Right Click to Use</p>
                 </div>
             </div>
        </div>
    );
